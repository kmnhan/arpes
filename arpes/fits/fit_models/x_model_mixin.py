...
class XModelMixin(lf.Model):
    ...
    def guess_fit(
        ...
        **kwargs
        ...
        kwargs.setdefault("coerce_farray", False)
        ...
        if not self.fit_flat:
            for i, k in enumerate(coord_values.keys()):
                coord_values[k] = coord_values[k][
                    tuple(None if i == j else slice(None) for j in range(self.n_dims))
                ]
        ...
        result = super().fit(
            flat_data if self.fit_flat else real_data,
            guessed_params,
            **coord_values,
    ...
    def xeval(self, params=None, **kwargs):
